...
Return ONLY the title:"""
    ...
    try:
        ...
        logger.warning("Rate limited during title extraction, using original query")
        return {
            'original': query,
            'clean': query,
            'year': year,
...
Return ONLY the English title, nothing else."""
    ...
    try:
        ...
        logger.warning("Rate limited during query translation, using original query")
        return query
...
def extract_conversation_context(history: list, current_query: str, language: str = "it") -> dict:
    ...
    for msg in history[-10:]:
        ...
        if content:
            ...
            recent_history.append(f"{prefix}: {content[:800]}")
...
Respond ONLY with valid JSON."""
    ...
    try:
        ...
        logger.warning("Rate limited during context extraction, defaulting to no context")
        return {
            'has_context': False,
            'context_summary': None,
            'referenced_films': [],
    ...
    if history:
        ...
        if context_result['should_use_context'] and context_result['referenced_films']:
            ...
            if len(referenced_films) == 1:
                ...
                for film_ref in referenced_films[:2]:
                    search_query = f"{film_ref['title']}"
                    if film_ref.get('year'):
                        search_query += f" {film_ref['year']}"
