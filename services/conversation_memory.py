...
Respond ONLY with valid JSON."""
    ...
    try:
        ...
        extracted = safe_json_parse(result_text, _empty_extraction)
        ...
        logger.warning("Rate limited during memory extraction, skipping this turn")
...
def update_memory(current_memory: dict, extracted_info: dict) -> dict:
    ...
    updated['key_moments'] = moments[-10:]
...
def get_memory_summary(memory: dict, language: str = "it") -> str:
    ...
    if memory.get('films_discussed'):
        films_text = ", ".join([
            ...
            for f in memory['films_discussed'][:5]
        ])
...
Respond ONLY with valid JSON."""
    ...
    try:
        ...
        result = safe_json_parse(result_text, _default)
        ...
        logger.warning("Rate limited during conversational check, defaulting to search")
        return _default
