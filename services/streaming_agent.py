...
Return ONLY the 2-letter code, nothing else."""
        ...
        logger.warning("Rate limited during language detection, defaulting to en")
        return 'en'
...
NO need to search for films - use what we've already discussed."""
            ...
            try:
                ...
                logger.warning("Rate limited during conversational response")
                yield {
                    "type": "complete",
                    "text": t("rate_limited", response_language),
                    "films": []
                }
                return
        ...
        if intent in ["info", "review"]:
            ...
            if (conversation_context and
                ...
                if results and len(results) > 0:
                    ...
                    yield {"type": "complete", "text": result["text"], "films": [result["film"]] if result["film"] else []}
                    return
...
- NEVER say "films you mentioned" - the user didn't mention any films"""
                ...
                try:
                    ...
                    logger.warning("Rate limited during cult films synthesis")
                    yield {
                        "type": "complete",
                        "text": t("rate_limited", response_language),
                        "films": all_films[:suggested_count]
                    }
                    return

                # Extract mentioned films from text
                mentioned_films = []
...
"""
        ...
        try:
            ...
            logger.warning("Rate limited during final recommendation synthesis")
            yield {
                "type": "complete",
                "text": t("rate_limited", response_language),
                "films": all_films[:3]
